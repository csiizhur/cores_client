 ...
 *
 */
public class AddAnatomy extends Application implements Initializable {
	...
	private static AddAnatomy instance;
	...
	public static AddAnatomy getInstance(){
		if(null == instance){
			instance =  new AddAnatomy();
		}
		return instance;
	}
	 ...
	 * @param taskIdList 
	 */
	public void loadData(List<String> taskIdList,List<String> sessionIdList,String id) {
		AddAnatomy.taskIdList = taskIdList;
		AddAnatomy.sessionIdList = sessionIdList;
...
//			OtherViscera.getInstance().start(getStage());
			...
			OtherViscera.getInstance().loadData("addAnatomy",taskId,studyNo,animalCode);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//					EditAnatomy.getInstance().loadData(null,"add",visceraMap,anatomyAnimal);
					EditAnatomy.getInstance().loadDataVisceraSelect(visceraMap, anatomyAnimal);
...
//					((javafx.scene.control.Control)event.getSource()).getScene().getWindow().hide();
	...
	public void start(Stage stage) throws Exception {
		Parent root = FXMLLoader.load(this.getClass().getResource("AddAnatomy.fxml"));
