		 ...
		 new BackgroundRunner() {
	            ...
	            public void background() throws Exception {
	                try {
	            		...
	            		list  = 	BaseService.getInstance().getUserService().findByPrivilegeName2("毒性病理_登录");
	            		data.clear();
...
//	                	}
	...
	private void onEnsureButtonAction(ActionEvent event) throws ParseException {
		...
		//			map.put("overdue", "");      // "" 表示用户密码未过期
			Map<String,Object> map = BaseService.getInstance().getUserService().checkPrivilege(userName, password,"毒性病理_登录");
			//判断用户是否为空
			if("".equals(map.get("nullUserError"))){
				...
				if("".equals(map.get("forbidden"))){
					...
					if("".equals(map.get("entitle"))){
						...
						tblLog.setOperatHost(SystemTool.getHostName()+","+SystemTool.getIPAddress());
						BaseService.getInstance().getTblLogService().save(tblLog);
						showErrorMessage(map.get("entitle").toString());
					}
				}else{
					//记录登录日志
					TblLog tblLog = new TblLog();
					...
					//账户已停用
					showErrorMessage(map.get("forbidden").toString());
				}
			}else{
				//记录登录日志
				TblLog tblLog = new TblLog();
				...
				if(Main.passwordErrorTimes<3){
					//用户为空
					showErrorMessage((String)map.get("nullUserError"));
					
					passwordText.setText("");
					userNameComboBox.requestFocus();
				}else{//密码录入错误次数达到3次
					showErrorMessage("密码输入错误达到3次");
					System.exit(1);
				}
			}
		}		
